...
The signal to promote something into a skill is boring. The same clarification keeps showing up in the instruction file, or the agent keeps getting the same multi step procedure slightly wrong in slightly different ways. For the Pinot agent, the first thing that hit this bar was launching a local cluster. The agent kept screwing up the startup sequence of the various components, bringing things up in the wrong order, forgetting a service or wiring the Java classpath for a binary slightly wrong so the process booted and then failed on the first query. Prose instructions couldn't pin any of that down, and every run burned time on the same category of mistake. Promoting it into a skill with an explicit startup procedure and a classpath helper script fixed it. Before that point, any earlier extraction would have been premature.
