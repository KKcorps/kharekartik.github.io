...
The single biggest lever on output quality in the Pinot agent wasn't the prompt or the tooling, it was the input. Most Pinot JIRAs are barebones. A two line description, a stack trace if you're lucky, filed because a commit needs a ticket number attached, not because anyone sat down to write a spec. A TPM driven ticket with clean acceptance criteria is the exception, not the norm. Hand the raw ticket to the agent and you get confident nonsense: a fix for the symptom the reporter pasted in, not the underlying change someone would actually merge.
...
For the Pinot agent the cleanest example was parsing Maven test output. Every eval bundle needed the `Tests run: N, Failures: M, Errors: E` line pulled out of Surefire logs. The agent kept writing ad hoc grep or sed to do it, usually slightly wrong, sometimes counting skipped tests as failures, sometimes missing the aggregate line entirely. Packaging that into a `parse_test_summary` shell helper meant one source of truth for what a passing build looked like. Same story for diff inventories and JaCoCo coverage extraction. Each one rewritten three or four times before I noticed, each one a line item now instead of a reasoning step.
